...
from pdfminer.pdfparser import PDFParser, PDFDocument, PDFStream
                        ...
                        for ch in item:
                            if isinstance(ch, LTChar):
                                ...
                                if len(text):
                                    ...
                                    # print(Rect(item.bbox, ph), item.get_text().strip())
                elif isinstance(item, LTImage):
                    r = Rect(item.bbox, ph)
                    data = item.stream.get_data()
                    # data = item.stream.get('Mask').resolve().get_rawdata()
                    try:
                        with io.BytesIO(data) as f:
                            img = Image.open(f)
                            img.load()

                            # data = item.stream.get_rawdata()
